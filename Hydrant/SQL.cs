...
internal class SQL
{
	...
	string Filter = "ORDER BY PUBLISH_DATE DESC")
	{
		...
		COMPANIES_MENTIONED, JAKE_FLAG, AUTHOR, SECTORS FROM ARTICLES {Filter} LIMIT {Limit}
		OFFSET {Offset};";
	...
	internal void UploadArticle(Article Article)
	{
		try
		{
			using (MySqlConnection connection = new(Connection))
			{
				...
				EXECUTIVES_MENTIONED, JAKE_FLAG, ImageURL, PUBLISHER_ID, AUTHOR, SECTORS) 
                VALUES 
                (@Title, @Url, @Content, @PublishDate, @RssSummary, @Summary, @Weighting,
				@Headline, @BusinessRelated, @Paywall, @CompaniesMentioned, 
				@ExecutivesMentioned, @JakeFlag, @ImageURL, @PUBLISHER_ID, @AUTHOR, @SECTORS)";
